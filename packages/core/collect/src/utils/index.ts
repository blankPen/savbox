import { replaceTag, copyComputedStyle } from "./dom";
export * from "./hash";
export * from "./dom";
...
export const snapshotDom = async (host: HTMLElement) => {
    const cloneNode = async (node: HTMLElement, parent: Element | DocumentFragment) => {
        const walkTree = async (nextn: HTMLElement | null, nextp: any) => {
            while (nextn) {
                await cloneNode(nextn, nextp);
                nextn = nextn.nextSibling as HTMLElement;
            }
        };
        ...
        if (node.shadowRoot) await walkTree(node.shadowRoot.firstChild as HTMLElement, clone);
        await walkTree(node.firstChild as HTMLElement, clone);
    };
